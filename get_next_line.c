...
int	validate(char **tmp, char **line, int fd)
{
	...
	if (tmp[fd][lenbefore] == '\n')
	{
		MFAIL((*line = ft_strsub(tmp[fd], 0, lenbefore)));
		MFAIL((after = ft_strdup(&tmp[fd][lenbefore + 1])));
		ft_strdel(&tmp[fd]);
		tmp[fd] = after;
		if (tmp[fd][0] == '\0')
			ft_strdel(&tmp[fd]);
...
int	get_next_line(const int fd, char **line)
{
	...
	while ((ret = read(fd, buf, BUFF_SIZE)) > 0)
	{
		buf[ret] = '\0';
		if (tmp[fd] == NULL)
			MFAIL((tmp[fd] = ft_strnew(1)));
		MFAIL((new = ft_strjoin(tmp[fd], buf)));
		ft_strdel(&tmp[fd]);
		tmp[fd] = new;
		if ((ft_strchr(buf, '\n')))
			break ;
